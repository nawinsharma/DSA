...
public class CeilingOfNumber {
    public static void main(String[] args) {
         ...
         * say target is 8, but target is not in the arr, this problem says that , hey,
         * 8 is not there so return the element ust greater than 8
         * which is in the array
         */
        int[] arr = { 2, 3, 5, 9, 14, 16, 18 };
        System.out.println(Ceiling(arr, 44));
...
// return the index of the smallest number >= target
    static int Ceiling(int[] arr, int target) {
        ...
        // when while loop breaks, start = end + 1;so no ans is found , so next big
        // number ( when no ans is found )is start only
        while (start <= end) {
